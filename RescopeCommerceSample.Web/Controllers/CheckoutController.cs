...
namespace RescopeCommerceSample.Web.Controllers
{
    ...
    public class CheckoutController : RenderController
    {
        ...
        public CheckoutController(ILogger<CheckoutController> logger, ICompositeViewEngine compositeViewEngine, IUmbracoContextAccessor umbracoContextAccessor, IBasketService basketService, IMemberService memberService, IPublishedValueFallback publishedValueFallback, MemberModelBuilderFactory memberModelBuilderFactory, IShippingCalculatorService shippingCalculatorService, IPaymentMethodService paymentMethodService, IOrderService orderService)
        ...
        public async Task<IActionResult> Index([FromQuery]string? stage, string? orderId)
        {
            ...
            var allowGuestCheckout = true;

            // if there isn't a valid basket for this user, and theyre not on the payment screen,
            // redirect them to the basket page
            var basket = await _basketService.GetCurrentBasket(CurrentPage!.AncestorOrSelf<Home>()!.RescopeStore!.Id);
            ...
            if (ViewData.HasPaymentHTML())
            {
                ...
                return View("Checkout/PaymentHTML",
            ...
            else if (stage == "payment")
            {
                ...
                { 
                    ...
                    {
                        return Redirect(CurrentPage.Parent!.FirstChild<Confirmation>()!.Url()
                            + "?orderId=" + HttpUtility.UrlEncode(order.OrderNumber)
                            + "&idHash=" + HttpUtility.UrlEncode(order.GetIdHash()));
                    }

                    ViewData.SetOrderId(order.Id);
                    return View("Checkout/PaymentRetry",
                       new CheckoutOrderViewModel(
                           order,
            ...
            else if (stage == "shipping")
            {
                return View("Checkout/Shipping",
                   new CheckoutViewModel<CheckoutShippingModel>(
                       ...
                       {
                           ShippingCalculationInfos = await _shippingCalculatorService.GetCalculations(basket) 
                       },
                       CurrentPage,
                       _publishedValueFallback)
                   {
            ...
            else if (!showLoginScreen || stage == "details" || isLoggedIn)
            {
                ...
                if (isLoggedIn)
                {
                    ...
                    if (checkout.DeliveryAddress != null && int.TryParse(User.Identity!.GetUserId(), out var id))
                    {
